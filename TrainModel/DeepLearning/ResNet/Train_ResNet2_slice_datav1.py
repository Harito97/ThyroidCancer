...
def plot_history(history):
    ...
    plt.savefig("slice_datav1_resnet2_model_accuracy_loss.png")
    # plt.show()
...
x = base_model(Inp) # (7, 7, 2048))
# x = Flatten(name="flatten")(x)  # duỗi ra 100352 đặc trưng
x = GlobalAveragePooling2D()(x) # còn lại 2048 đặc trưng
x = Dense(1024 * 3, activation="relu")(x)
x = Dropout(0.5)(x)
...
for layer in finetuned_model.layers[-freeze_layers:]:
    layer.trainable = False

# finetuned_model.load_weights('D:/Projects/ThyroidCancer/TrainModel/DeepLearning/ResNet/Train_result/resnet1_best_slice_datav1.keras')
finetuned_model.load_weights('D:/Projects/ThyroidCancer/TrainModel/DeepLearning/ResNet/Train_result/resnet2_best_slice_datav1.keras')
...
checkpointer = ModelCheckpoint(
    "resnet2_best_slice_datav1.keras",
    verbose=1,
    save_best_only=True,
)
...
# )
# finetuned_model.save("resnet2_best_slice_datav1_trainalllayer.keras")
# save_history(History)
# plot_history(History)
# end2 = time.process_time()
# print("final is in ", end2 - start)

print("Bắt đầu train lại toàn bộ bao gồm cả tầng cuối cùng")
...
def save_history1(history):
    acc = pd.Series(history.history["accuracy"], name="accuracy")
    loss = pd.Series(history.history["loss"], name="loss")
    val_acc = pd.Series(history.history["val_accuracy"], name="val_accuracy")
    val_loss = pd.Series(history.history["val_loss"], name="val_loss")
    com = pd.concat([acc, loss, val_acc, val_loss], axis=1)
    com.to_csv("slice_datav1_vgg2_history_trainalllayer.csv", index=False)
...
def plot_history1(history):
    ...
    plt.xlabel("Epoch")
    plt.legend(["accuracy", "val_accuracy", "loss", "val_loss"], loc="upper right")
    plt.savefig("slice_datav1_resnet2_model_accuracy_loss_trainalllayer.png")
    # plt.show()

save_history1(History)
plot_history1(History)
end2 = time.process_time()
print("final is in ", end2 - start)
# finetuned_model.save("resnet2_best_slice_datav1.keras")
